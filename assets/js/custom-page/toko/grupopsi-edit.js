...
import deleteOpsi from "../../request/opsi.js";
...
const loadPage = async () => {
    try {
        ...
        displayContent();
        document.getElementById('goback').href = `/${slugToko}/grup-opsi`
...
const displayContent = async () => {
    try {
        ...
        document.getElementById("required").checked = result.required;
        document.getElementById("spesificRequest").checked = result.spesificRequest;
        document.getElementById("min").value = result.min;
        document.getElementById("max").value = result.max;
        document.getElementById("hardcopy").checked = result.hardcopy;
        document.getElementById("softcopy").checked = result.softcopy;
        
        if (result.opsi === null) {
            return;
        }
        let htmlOpsi = ``;
        result.opsi.forEach(v => {
            htmlOpsi += `<div class="form-row opsi" id="opsi-${v.idOpsi}">
            <div class="form-group col-md-6 col-12 col-lg-6">
                <input type="text" class="idOpsi" value="${v.idOpsi}" hidden>
                <label class="col-form-label text-md-right">Nama Item <small>(maksimal 50 karakter)</small></label>
                <input type="text" class="form-control namaOpsi" value="${v.opsi}"/>
            </div>
            <div class="form-group col-md-3 col-6 col-lg-3">
                <label class="col-form-label text-md-right">Harga <small>(rupiah)</small></label>
                <input type="text" class="form-control harga currency" value="${v.harga}"/>
            </div>
            <div class="form-group col-md-3 col-6 col-lg-3">
                <label class="col-form-label text-md-right">Berat <small>(gram)</small></label>
                <input type="text" class="form-control berat currency" value="${v.berat}"/>
            </div>
            <div class="form-group col-md-4 col-7 col-lg-4">
                <label class="col-form-label text-md-right">Kelipatan tiap pembelian produk? 
            ...
            <div class="form-group col-md-2 col-5 col-lg-2">
                ...
                <select class="form-control status">
                    <option value="false" `+
                    (v.perProduk == false ? 'selected="selected"' : '') +`>Tidak Tersedia</option>
                    <option value="true" `+
                    (v.status == true ? 'selected="selected"' : '') +`>Tersedia</option>
                </select>
            </div>
            <div class="form-group col-md-1 col-1 col-lg-1">
                <label class="col-form-label text-md-right">Aksi</label><br>
                <a href="#" class="btn btn-icon btn-sm btn-danger mt-1" onclick="hapusOpsi(${v.idOpsi})" data-toggle="tooltip" data-placement="top" title="Hapus"><i class="far fa-trash-alt"></i></a>
            </div>
        </div>`;
        });
        document.getElementById('data-opsi').innerHTML = htmlOpsi;
    } catch(error) {
...
const hapusOpsi = async (idOpsi) => {
    try {
        ...
        if (confirm == true) {
            ...
            let result = deleteOpsi(idToko, idGrupOpsi, idOpsi);
...
const ubahGrupOpsi = async () => {
    ...
    try {
        ...
        let result = await grupOpsi.updateGrupOpsi(idToko, idGrupOpsi, jsonData);
        alertSuccess(result.message);
    } catch(error) {
        alertFailed(error, false)
    } finally {
...
window.ubahGrupOpsi = ubahGrupOpsi;
